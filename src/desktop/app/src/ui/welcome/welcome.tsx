...
import { SignInCustom } from './sign-in-custom'
...
export enum WelcomeStep {
  Start = 'Start',
  SignInToOfficialWithBrowser = 'SignInToDotComWithBrowser',
  SignInToCustom = 'SignInToCustom',
...
export class Welcome extends React.Component<IWelcomeProps, IWelcomeState> {
  ...
  private get inSignInStep() {
    if (this.state.currentStep === WelcomeStep.SignInToOfficialWithBrowser) {
      return true
    }

    if (this.state.currentStep === WelcomeStep.SignInToCustom) {
      return true
    }
  ...
  private getComponentForCurrentStep() {
    ...
    switch (step) {
      ...
      case WelcomeStep.SignInToOfficialWithBrowser:
        const loadingBrowserAuth =
          step === WelcomeStep.SignInToOfficialWithBrowser &&
          signInState !== null &&
      ...
      case WelcomeStep.SignInToCustom:
        return (
          <SignInCustom
            dispatcher={this.props.dispatcher}
            advance={this.advanceToStep}
  ...
  private advanceToStep = (step: WelcomeStep) => {
    ...
    if (step === WelcomeStep.SignInToCustom) {
