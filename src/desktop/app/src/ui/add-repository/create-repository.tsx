...
import { IIgnore, getIgnoreNames, writeIgnore } from './ignores'
...
interface ICreateRepositoryState {
  ...
  readonly ignores: ReadonlyArray<IIgnore> | null
...
> {
  ...
  public async componentDidMount() {
    ...
    const ignoreNames = await getIgnoreNames()
    const licenses = await getLicenses()

    this.setState({ ignores: ignoreNames, licenses })
  ...
  private createRepository = async () => {
    ...
    if (ignore !== NoIgnoreValue.name) {
      try {
        await writeIgnore(fullPath, ignore)
      } catch (e) {
        log.error(
  ...
  private onIgnoreChange = (event: React.FormEvent<HTMLSelectElement>) => {
    const gitIgnore = event.currentTarget.value
    this.setState({ ignore: gitIgnore })
  }
  ...
  private renderIgnores() {
    const ignores = this.state.ignores || []
    const featuredIgnores = [NoIgnoreValue, ...ignores.filter(l => l.featured)]
    const nonFeaturedIgnores = ignores.filter(l => !l.featured)
    ...
    return (
      <Row>
        ...
        >
          {featuredIgnores.map(l => (
            <option key={l.name} value={l.name}>
              {l.name}
            </option>
          ))}
          <option disabled={true}>────────────────────</option>
          {nonFeaturedIgnores.map(l => (
            <option key={l.name} value={l.name}>
              {l.name}
            </option>
          ))}
