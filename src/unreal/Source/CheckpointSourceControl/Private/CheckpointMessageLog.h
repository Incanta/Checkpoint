...
public:
  ...
  FTSMessageLog(const FName &InLogName) : Log(InLogName) {}
  ...
  TSharedRef<FTokenizedMessage> Message(
    EMessageSeverity::Type InSeverity, const FText &InMessage = FText()
  ) {
    if (IsInGameThread()) {
      return Log.Message(InSeverity, InMessage);
    } else {
      TSharedRef<FTokenizedMessage> Message =
        FTokenizedMessage::Create(InSeverity, InMessage);
      UE_LOG(
        LogSourceControl, Display, TEXT("%s"), *Message->ToText().ToString()
      );
  ...
  TSharedRef<FTokenizedMessage> Error(const FText &InMessage = FText()) {
    if (IsInGameThread()) {
      ...
      TSharedRef<FTokenizedMessage> Message =
        FTokenizedMessage::Create(EMessageSeverity::Error, InMessage);
      UE_LOG(
        LogSourceControl, Error, TEXT("%s"), *Message->ToText().ToString()
      );
  ...
  TSharedRef<FTokenizedMessage> PerformanceWarning(
    ...
    if (IsInGameThread()) {
      ...
      TSharedRef<FTokenizedMessage> Message = FTokenizedMessage::Create(
        EMessageSeverity::PerformanceWarning, InMessage
      );
      UE_LOG(
        LogSourceControl, Warning, TEXT("%s"), *Message->ToText().ToString()
      );
  ...
  TSharedRef<FTokenizedMessage> Warning(const FText &InMessage = FText()) {
    if (IsInGameThread()) {
      ...
      TSharedRef<FTokenizedMessage> Message =
        FTokenizedMessage::Create(EMessageSeverity::Warning, InMessage);
      UE_LOG(
        LogSourceControl, Warning, TEXT("%s"), *Message->ToText().ToString()
      );
  ...
  TSharedRef<FTokenizedMessage> Info(const FText &InMessage = FText()) {
    if (IsInGameThread()) {
      ...
      TSharedRef<FTokenizedMessage> Message =
        FTokenizedMessage::Create(EMessageSeverity::Info, InMessage);
      UE_LOG(
        LogSourceControl, Display, TEXT("%s"), *Message->ToText().ToString()
      );
