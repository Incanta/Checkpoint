...
export default function IssueDetailPage() {
  ...
  useDocumentTitle(
    issue
      ? `${issue.title} #${issue.number} - ${repoName}`
      : `Issue #${issueNumber}`,
  );
  ...
  if (!issue) {
    return (
      <EmptyState
        ...
        description={`Could not find issue #${issueNumber}.`}
      />
    );
  }

  const currentLabelIds = new Set(issue.labels?.map((ll) => ll.label.id) ?? []);
    ...
    <div>
      ...
      <div className="flex gap-6">
        ...
        <div className="w-56 shrink-0 space-y-4">
          {/* Labels */}
          <Card>
            <h3 className="mb-2 text-xs font-semibold text-[var(--color-text-muted)] uppercase">
              Labels
            </h3>
            ...
            {allLabels &&
              allLabels.filter((l) => !currentLabelIds.has(l.id)).length >
                0 && (
                <select
                  value=""
                  onChange={(e) => {
          ...
          <Card>
            ...
            {members &&
              members.filter((m) => !currentAssigneeIds.has(m.user.id)).length >
                0 && (
                <select
                  value=""
                  onChange={(e) => {
                ...
                >
                  ...
                  {members
                    .filter((m) => !currentAssigneeIds.has(m.user.id))
                    .map((m) => (
