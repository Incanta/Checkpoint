...
export default function OrgPage() {
    ...
    return (
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mt-6 space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-16 animate-pulse rounded-lg bg-[var(--color-bg-secondary)]" />
          ))}
        </div>
      </div>
    );
  ...
  const isAdmin = (org as any).users?.some(
      ...
      <PageHeader
        title={
          <span className="flex items-center gap-3">
            ...
            {(org as any).users && (
              <Badge>
                {(org as any).users.length} member{(org as any).users.length !== 1 ? "s" : ""}
              </Badge>
            )}
          </span>
        }
              ...
              <Card className="transition-colors hover:border-[var(--color-border-default)] hover:bg-[var(--color-bg-surface)]">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor" className="text-[var(--color-text-muted)]">
